...
B=$BENCH_HOME/lib
L=$FABAN_HOME/lib
CLASSPATH=$B/mysql-connector-java-5.0.6-bin.jar:$B/JSON.jar:$B/JavaOlioDriver.jar:\
...
"jdbc:mysql://$DB_HOST/bpwebapp?user=olio&password=olio&relaxAutoCommit=true&sessionVariables=FOREIGN_KEY_CHECKS=0" $SCALE
EXIT_CODE=$?
if [ "$EXIT_CODE" = 0 ] ; then
