    ...
    module Fragments
      ...
      def write_fragment(name, content, options = nil)
        ...
        self.class.benchmark "Cached fragment: #{key}" do
          ApplicationController.local_fragment_cache[key] = content
          return false if !fragment_cache_store.write(key, content, options)
        end
        content
      end
    end
    ...
    module Fragments
      ...
      def cache_erb_fragment(block, name={}, options=nil, interpolation={})
        ...
        if cache = read_fragment(name, options)
          ...
          pos = buffer.length
          block.call
          if write_fragment(name, buffer[pos..-1], options)
...
class MemCache
  ...
  def read(key, options=nil)    
    begin
      value = get(key)
      #replace(key, value, 30) unless value.nil?
      set(key, value, 30) unless value.nil?
      value
    rescue 
  ...
  def write(key, content, options=nil)
    expiry = options && options[:expire] || 30.seconds
    begin
      get_lock = add("lock:" + key, "", 5)
      if get_lock.strip! == "STORED"
